...
function Example() {
  ...
  useEffect(() => {
    document.querySelector(
      ".effect"
    ).innerHTML = `you clicked ${count} times. From effect`;
...
class FriendContainer extends Component {
  ...
  componentDidMount() {
    let intervalId = setInterval(() => {
      let randomFriend =
        this.state.friends[Math.floor(Math.random() * FRIENDS.length)];
            ...
            {this.state.friends.map((friend, index) => (
              <div key={friend.id} className="friend m-3 d-flex ">
                ...
                <div style={{ marginLeft: "auto" }}>
                  <h5>{friend.name}</h5>
                  <span className="friend-id" style={{ display: "none" }}>
                    {friend.id}
                  </span>
                  <button
                    onClick={() => this.subscribeToFriend(friend)}
                    className="btn bg-primary"
                  >
                    subscribe
                  </button>
                  <button
                    onClick={() => this.unSubscribeToFriend(friend)}
...
friendsApi.subscribeToFriendStatus = function (id, callback) {
  this.getFriend(id, callback);
};

friendsApi.getFriend = function (id, callback) {
...
friendsApi.unsubscribeFromFriendStatus = function (id, callback) {
  this.getFriend(id, callback);
};
...
function FriendListItem(props) {
  ...
  const isOnline = useFriendStatus(friendsApi, props.friend.id);
...
const CommentWithSubscription = withSubscription(CommentList, (DataSource) =>
  ...
  render() {
    return (
      <div className="col-sm-7 mouse-move-wrapper">
        <div
          className="mouse-container p-3 rounded m-2"
          style={{
            height: "70vh",
          }}
        ...
        >
          <p>
            Current mouse position x: ({this.state.x}, {this.state.y})
          </p>
...
class Cat extends Component {
  render() {
    ...
    return (
      <img
        src="https://www.clipartmax.com/png/middle/27-270383_kitten-cartoon-drawings-of-animals-clipart-image-cat-png-clip-art.png"
        alt=""
        style={{
          height: "30px",
          width: "30px",
